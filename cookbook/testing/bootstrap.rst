...
（訳注：これは、環境変数 ``$ENV['BOOTSTRAP_CLEAR_CACHE_ENV']`` に設定された環境のキャッシュをクリアして、オリジナルのブートストラップファイルをロードするだけの簡単なスクリプトです。）
...
これで、テストの実行前に ``test`` 環境のキャッシュをクリアするブートストラップ処理が実行されるようになります。
