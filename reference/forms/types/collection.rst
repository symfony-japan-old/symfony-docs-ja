...
最も簡単なところでは、 ``emails`` フィールドに入力する ``text`` フィールドの配列かもしれません。
より複雑な例では、フォーム全体を埋め込むことができ、それは、一対多の関係を出力するフォームを作成するときに便利です（たとえば、多くの関連製品の写真を管理する場所からの製品）。
...
たとえば、メールアドレス配列に対応する ``emails`` フィールドがあるとします。フォーム中ではメールアドレス毎に独自のテキストボックスを出力したいです::
...
どちらの場合も ``emails`` データ配列がすでにいくつかのメールアドレスを含んでいなければ入力フィールドは表示されません。

この簡単な例では、まだ新しいアドレスを追加することや既存のアドレスを削除することはできません。
新しくアドレスを追加するには `allow_add`_ オプション（および `prototype`_ オプション）を有効にすることで可能です。
(続く例を見てください)。メールアドレスを ``emails`` 配列から削除することは `allow_delete`_ オプションで可能です。
...
あなたのユーザーが別のメールを追加できるようにするために、まず `allow_add`_ を ``true`` に設定します。そして、
- JavaScript 経由で - ``form[emails][2]`` という名前で他のフィールド（など、よりおおくのフィールドを）を表示します。

これを容易にするのを助けるため、 `prototype`_ オプションを ``true`` にすることで"テンプレート" フィールドの表示を許可します。
それは JavaScript を利用して動的に新しいフィールドを作成することが出来ます。
