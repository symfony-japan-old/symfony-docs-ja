...
現実を認めよう。\
どんなアプリケーションにとっても、データベースへの情報の永続化やデータベースからの情報の取得を行うということは、\
最もよく使わ、そしてチャレンジしがいのあるタスクだ。\
...
.. note::
    ...
    データベースに直にクエリするほうが良いのであれば、それも簡単で、クックブックの ":doc:`/cookbook/doctrine/dbal`" で説明しています。

    Doctrine ODM ライブラリを使って `MongoDB`_ に永続化することも可能です。\
    詳細は、クックブックの"\ :doc:`/cookbook/doctrine/mongodb`\ "を参照してください。

簡単な例: 商品
--------------

どのように Doctrine が動作するのか理解するのに一番簡単なのは、実際に動作しているのを見ることでしょう。\
この節では、データベースの設定行い、そして、\ ``Product(商品)`` オブジェクトを作成して、\
データベースへの永続化を行い、反対にそれをフェッチしてみます。

.. sidebar:: 例を使ってにコードを追う

    この章で出てくる例を実際に追っていくには、\ ``AcmeStoreBundle`` を作成してください。
...
Doctrine がDB情報を把握することができたので、\
Doctrine にデータベースをつくらせてみましょう。
...
.. tip::
    ...
    次のようにしても、エンティティクラスを作成させることができます。
...
Doctrine では、単にカラムベースのテープルを配列にしてフェッチするといったやり方よりも\
...
Doctrine がこの作業をできるようにするには、"metadata" を作成するか、\
``Product`` クラスとそのプロパティがデータベースにどの様にマップされるのか、という設定を作成するだけです。\
metadata は、色々なフォーマットで記述することが可能で、\
YAML や XML、もしくは、アノテーションとして ``Product`` クラスに直接記述することもできます。

.. note::

    １つのバンドルでは、metadata の定義は1つのフォーマットでしかできません。\
...
.. tip::
    ...
    テーブル名はオプションで、もし省略された場合は、\
    エンティティクラス名に基づいて自動的に決定されます。
...
ありがたい事に、次を実行すると、これを Doctrine がやってくれます。
...
ですが、データベースには、まだ、これに対応する ``product`` テーブルがありません。\
...
.. tip::
    ...
    *今どうなっている*\ のかということを比較して、\
    ...
    さらにこの機能をうまく使うには、\ :doc:`マイグレーション</cookbook/doctrine/migrations>` を通すことでしょう。\
...
.. note::
  ...
  可能なかぎりクエリが効率的に成るように実行します。\
  例えば、100個の ``Product`` オブジェクトを永続化しようと ``persist()`` を呼ぶと、\
...
.. tip::
    ...
    詳細は、\ :doc:`/cookbook/doctrine/doctrine_fixtures` を参照してください。
...
ある特定の種類のオブジェクトに対するクエリの場合は、"repository(リポジトリ)" と呼ばれるものを使います。\
リポジトリは、特定のクラスのエンティティをフェッチしてくるのを補助するためだけの PHP クラスと考えてよいでしょう。\
あるエンティティクラスに対するリポジトリオブジェクトへのアクセスは次のように行います。\ ::
...
.. caution::
    ...
    それぞれ、\ ``Doctrine\ORM\NoResultException``\ 、\ ``Doctrine\ORM\NonUniqueResultException`` を投げます。\
    もしこのメソッドを使用する場合は(そして、1つより多くの結果を返すようなクエリをしている場合は)、try-catch ブロックで囲って、\
...
もう一度行っておきますが、これは Doctrine が必要とするわけではありません。\
...
続きを見ていく前に、Doctrine に ``category`` テーブル、\ ``product.category_id`` カラム、そしてフォーリンキーを追加させるのを忘れないで下さい。
...
.. note::
    ...
    プロダクション環境のデータベースをより堅牢にそしてシステマチックに更新する際は、\ :doc:`Doctrine migrations</cookbook/doctrine/migrations>` を参照してください。
...
.. note::
   ...
   詳細は、クックブックの ":doc:`/cookbook/doctrine/doctrine_fixtures`" を参照してください。
