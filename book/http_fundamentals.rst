...
同じリソースを返すにしても、例えば HTML や XML、JSON といった様々な返し方がりますよね。
...
#. 実用本位な設定ができ、上記のバラバラになっている部品群を紐付けする"グル"としての役割を提供すること

フレームワークの目的は、独立したツール群を統合し、開発者に一貫したエクスピリエンスを提供することです。
フレームワークそれ自体も、Symfony2 バンドル(プラグイン)であり、設定可能で、まるっと置き換えることすら可能になっています。

Symfony2 は強力なツール群を提供しており、無理を強いること無く、迅速な WEB アプリケーションの開発が可能にっています。
一般的なユーザであれば、実用的なデフォルト値が設定済みのスケルトンプロジェクトが入っている Symfony2 ディストリビューションが準備されているので、簡単に開発を始めることができます。
より踏み込んだユーザ? 制限なんてありませんよ。
